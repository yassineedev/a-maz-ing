...
static char	**mlx_int_str_to_wordtab(char *str)
{
  char	**tab;
  int	pos;
  int	nb_word;
...
static char	*mlx_int_static_line(char *xpmdata,int *pos,int size)
{
  static char	*copy = 0;
  static int	len = 0;
  int		len2;
...
static void	mlx_int_xpm_set_pixel(unsigned char *data, int opp, int col, int x)
{
  *((unsigned int *)(data+4*x)) = col;
}


static void	*mlx_int_parse_xpm(void *xvar,void *info,int info_size,char *(*f)(char *, int *, int),
				   unsigned int *width, unsigned int *height)
{
  ...
  if (!(img = mlx_new_image(xvar,*width,*height)))
    RETURN;
  data = mlx_get_data_addr(img, &opp, &sl, &format);
  opp = 4;
...
void	*mlx_xpm_file_to_image(void *xvar,const char *file, unsigned int *width, unsigned int *height)
{
  int	fd;
  int	size;
  char	*ptr;
...
void	*mlx_xpm_to_image(void *xvar, const char **xpm_data, unsigned int *width, unsigned int *height)
{
