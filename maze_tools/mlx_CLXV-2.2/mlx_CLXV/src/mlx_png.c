...
static int	mipng_is_type(unsigned char *ptr, char *type)
{
...
static unsigned char mipng_defilter_none(unsigned char *buff, int pos, int a, int b, int c)
{ return (buff[pos]); }
static unsigned char mipng_defilter_sub(unsigned char *buff, int pos, int a, int b, int c)
{ return (buff[pos]+(unsigned int)a); }
static unsigned char mipng_defilter_up(unsigned char *buff, int pos, int a, int b, int c)
{ return (buff[pos]+(unsigned int)b); }
static unsigned char mipng_defilter_average(unsigned char *buff, int pos, int a, int b, int c)
{ return (buff[pos]+((unsigned int)a+(unsigned int)b)/2); }
static unsigned char mipng_defilter_paeth(unsigned char *buff, int pos, int a, int b, int c)
{
  //  int	p;
  int	result;
...
static unsigned char (*(mipng_defilter[]))(unsigned char *buff, int pos, int a, int b, int c) =
{
  mipng_defilter_none,
  mipng_defilter_sub,
  mipng_defilter_up,
...
static int	mipng_fill_img(void *img, unsigned char *buf, png_info_t *pi)
{
  ...
  ibuf = (unsigned char *)mlx_get_data_addr(img, &bpp, &iline, &format);
...
static int	mipng_data(void *img, unsigned char *dat, png_info_t *pi)
{
  unsigned int	len;
  int		b_pos;
...
static int	mipng_magic(unsigned char *ptr, int size)
{
  int	i;
...
static int	mipng_crc(unsigned char *ptr, int len)
{
...
static int	mipng_structure(unsigned char *ptr, int size, unsigned char **hdr, unsigned char **dat)
{
  unsigned int	len;
  int		dat_state;
  int		end;
...
static int	mipng_verif_hdr(unsigned char *hdr, png_info_t *pi)
{
...
static void	*mlx_int_parse_png(void *xvar, unsigned char *fptr, int size, unsigned int *width, unsigned int *height)
{
  int		err;
  unsigned char *hdr;
  unsigned char *dat;
...
void	*mlx_png_file_to_image(void *xvar, const char *filename,
			       unsigned int *width, unsigned int *height)
{
  ...
  if ((fd = open(filename, O_RDONLY)) == -1 || (size = lseek(fd, 0, SEEK_END)) == -1 ||
      (ptr = mmap(0, size, PROT_READ, MAP_PRIVATE, fd, 0)) == (void *)MAP_FAILED)
    {
      if (fd >= 0)
        close(fd);
      warnx("Can't map png file '%s'", filename);
      return ((void *)0);
    }
